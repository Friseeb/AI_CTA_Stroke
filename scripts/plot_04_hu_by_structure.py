#!/usr/bin/env python
"""
plot_04_hu_by_structure.py — Median HU by structure and CT type.
Produces: raincloud_hu_by_structure.png
"""
...
# ── Plot ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description='Median HU by structure raincloud')
    ...
    aorta_col = 'aorta_original_firstorder_Median'
    print("\nCases with Aorta Median HU < 150:")
    for ct in ct_types:
        vals = df.loc[df['ct_type'] == ct, aorta_col].dropna()
        n_below = (vals < 150).sum()
        print(f"  {ct}: {n_below} / {len(vals)}")
    ...
    for ax, (struct, col) in zip(axes, structures):
        ...
        if struct == 'Aorta':
            ax.axvline(150, color='#cc0000', lw=1.3, ls='--',
                       label='Contrast threshold (150 HU)', zorder=2)
