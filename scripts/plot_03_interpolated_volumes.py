#!/usr/bin/env python
"""
plot_03_interpolated_volumes.py — Interpolated voxel count with QC thresholds.
Produces: raincloud_interpolated_volumes.png
"""
...
# ── Plot ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description='Interpolated voxel count raincloud with QC thresholds')
    ...
    structures = [
        ('LAA',   'laa_diagnostics_Mask-interpolated_VoxelNum',
         dict(hard=1.0,  soft=5.0)),
        ('LA',    'la_diagnostics_Mask-interpolated_VoxelNum',
         dict(hard=10.0, soft=50.0)),
    ...
    for ax, (struct, col, thresh) in zip(axes, structures):
        data_dict = {ct: df.loc[df['ct_type'] == ct, col].dropna().values / 1000.0
                     for ct in ct_types}
        ...
        if thresh:
            ax.axvline(thresh['hard'], color='#cc0000', lw=1.3, ls='--',
                       label=f"Hard = {thresh['hard']} mL", zorder=2)
            ax.axvline(thresh['soft'], color='#e68a00', lw=1.3, ls=':',
                       label=f"Soft = {thresh['soft']} mL", zorder=2)
            ax.legend(fontsize=7.5, loc='upper right', framealpha=0.75)

    fig.suptitle('Interpolated Volume by Structure and CT Type',
