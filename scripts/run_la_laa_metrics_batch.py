...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=None,
        action="store_true",
        help="Show tqdm progress bar (default: auto-detect terminal).",
    )
    p.add_argument(
        "--resume",
        action="store_true",
...
def find_first_mesh(case_dir: Path, suffix: str) -> Path | None:
    ...
    # Search case root first, then the surfaces/ subfolder created by
    # run_laa_shape_descriptors.py (so this script works whether or not
    # run_batch_la_metrics.py has already copied the files up).
    for search_dir in (case_dir, case_dir / "surfaces"):
...
def _compute_case_row_isolated(
    ...
    if float(case_memory_limit_gb) > 0:
        ...
        def _set_limits() -> None: pass  # resource limits not available on Windows
...
def main() -> int:
    ...
    use_tqdm = sys.stdout.isatty() if args.progress is None else bool(args.progress)
    _tqdm_bar = None
    work_iter = work
    if use_tqdm:
        try:
            from tqdm.auto import tqdm as _tqdm_cls
            _tqdm_bar = _tqdm_cls(work, total=len(work), unit="case", desc="LA/LAA metrics")
            work_iter = _tqdm_bar
        except Exception:
            print("tqdm not available; falling back to plain progress prints.")
            use_tqdm = False
    ...
    for i, (case_id, sid, la_path, laa_path) in enumerate(work_iter, start=1):
        elapsed_total = time.perf_counter() - batch_t0
        eta = elapsed_total / (i - 1) * (len(work) - i + 1) if i > 1 else 0.0
        if not use_tqdm:
            print(
                f"[{i}/{len(work)}] {case_id}"
                f"  |  elapsed {_fmt(elapsed_total)}  ETA {_fmt(eta) if i > 1 else '?'}",
                flush=True,
            )
        ...
        elapsed_total = time.perf_counter() - batch_t0
        eta = elapsed_total / i * (len(work) - i)
        if not use_tqdm:
            print(
                f"  → {status_str}  case {dt:.1f}s | total {_fmt(elapsed_total)} | ETA {_fmt(eta)}",
                flush=True,
            )
        elif _tqdm_bar is not None:
            _tqdm_bar.set_postfix(
                status=status_str,
                case_s=f"{dt:.1f}s",
                total=_fmt(elapsed_total),
                eta=_fmt(eta),
                refresh=True,
            )
