#!/usr/bin/env python
"""
plot_07_laa_aorta_ratio.py — LAA/Aorta median HU ratio (LAA-valid rows only).
Produces: raincloud_laa_aorta_ratio.png
"""
...
# ── Plot ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description='LAA/Aorta median HU ratio raincloud')
    ...
    df_valid  = df[df[col_laa].notna()]
    print(f"Rows with valid LAA features: {len(df_valid)}")

    fh = _fig_height(len(ct_types))
    fig, ax = plt.subplots(figsize=(7.5, fh))

    data_dict = {
        ct: df_valid.loc[df_valid['ct_type'] == ct, col_ratio].dropna().values
        for ct in ct_types
    }
