#!/usr/bin/env python
"""
plot_01_roi_voxels.py — ROI voxel count (pre-resampling), log scale.
Produces: raincloud_roi_voxels.png
"""
...
# ── Plot ──────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description='ROI voxel count raincloud (pre-resampling)')
    ...
    for ax, (struct, col) in zip(axes, structures):
        ...
        _setup_ax(ax, ct_types, data_dict, ct_types, log_scale=True)
