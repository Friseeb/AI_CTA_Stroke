...
```bash
export CTA_WORKSPACE=/path/to/AI_CTA_Stroke
export AORTA_REPO="$CTA_WORKSPACE/aorta_cta_radiomics"
conda env create -f "$AORTA_REPO/environment.yml"
conda activate aorta-cta-radiomics
python -m pip install -e "$CTA_WORKSPACE/cta_common"
python -m pip install -e "$AORTA_REPO[dev,table]"
```

Optional mesh or MONAI dependencies are intentionally separate:

```bash
conda install -c conda-forge vtk pyvista trimesh
pip install monai torch
```

`cta_common` is a required workspace package. It is installed explicitly before
the aorta package because it is not published by this workspace to a package
index. `pyarrow` is optional; without it, sample-point output
uses a manifest-declared CSV fallback.
...
## Carotid Perivascular Milestone

The package now contains an additive, FLOWCAT-backed first milestone for
vessel-centred perivascular phenotyping of bilateral common carotid and cervical
internal carotid arteries. It consumes native FLOWCAT segmentation, centreline,
branch, and anatomical-graph products; it does not run a substitute arterial
pipeline or infer missing vessel names.

Implemented components include:

- safe typed discovery/loading and SHA-256 manifests for FLOWCAT outputs;
- branch-aware anatomical label propagation with explicit uncertainty;
- physical centreline resampling and rotation-minimizing vessel frames;
- fixed and capped diameter-adaptive physical-mm shells;
- deterministic fat, muscle, vein, bone, thyroid, airway, residual-soft-tissue,
  high-density, and uncertain compartments;
- radial, longitudinal, angular-sector, and arterial-surface-contact summaries;
- case-, vessel-, and sample-point-level machine-readable outputs; and
- headless integrity/visual-QC generation.

The operational carotid bulb is not a FLOWCAT class. It is only permitted as an
evidence-backed configured longitudinal interval. Missing coarse anatomy remains
explicitly unavailable and forces manual review.

### Required native FLOWCAT products

A runnable case directory must contain the following under
`extracranial_vessels/`:

```text
segmentation.nii.gz
branch_model.vtk              # .vtp is also accepted
centerline_segments_array.npy
segments_graph_pred.pickle
local_graph.pickle
```

The NumPy object array and NetworkX pickles are executable serialization. Use
`--trusted-flowcat-artifacts` only for outputs generated by a controlled local
run of the pinned FLOWCAT checkout. A checksum proves file identity, not safety
...
export CTA_WORKSPACE=/path/to/AI_CTA_Stroke
export AORTA_REPO="$CTA_WORKSPACE/aorta_cta_radiomics"
export FLOWCAT_REPO="$CTA_WORKSPACE/external/arterial_flowcat"
export CASE_ID=sub-DEIDENTIFIED_acq-CTA_ct
export FLOWCAT_CASE=/path/to/flowcat-output/$CASE_ID
export CTA_NIFTI=/path/to/$CASE_ID.nii.gz
export CAROTID_OUT=/path/to/carotid-output/$CASE_ID
```

Audit without opening pickle or object-array contents:

```bash
python "$AORTA_REPO/scripts/audit_carotid_pipeline.py" \
  --case-dir "$FLOWCAT_CASE" \
  --case-id "$CASE_ID" \
  --pretty
```

Create the persistent SHA-256 output manifest:

```bash
python -m aorta_cta_radiomics.carotid_cli manifest \
  --case-dir "$FLOWCAT_CASE" \
  --case-id "$CASE_ID" \
  --flowcat-repo "$FLOWCAT_REPO" \
  --output "$CAROTID_OUT/flowcat_output_manifest.json"
```

Run the deterministic carotid analysis after reviewing that manifest:

```bash
python -m aorta_cta_radiomics.carotid_cli run \
  --case-dir "$FLOWCAT_CASE" \
  --case-id "$CASE_ID" \
  --cta "$CTA_NIFTI" \
  --flowcat-repo "$FLOWCAT_REPO" \
  --config "$AORTA_REPO/config/carotid_perivascular.yaml" \
  --outdir "$CAROTID_OUT" \
  --trusted-flowcat-artifacts
```

Optional geometry-matched binary masks can be supplied with `--vein-mask`,
`--bone-mask`, `--airway-mask`, `--thyroid-mask`, `--muscle-mask`,
`--uncertain-mask`, and `--high-density-mask`. The run writes a failure record
and returns exit status 2 when required native products are absent or invalid.

Generate headless QC even for an incomplete binary-only case:

```bash
python -m aorta_cta_radiomics.carotid_qc \
  --case-id "$CASE_ID" \
  --cta "$CTA_NIFTI" \
  --segmentation "$FLOWCAT_CASE/extracranial_vessels/segmentation.nii.gz" \
  --flowcat-manifest "$CAROTID_OUT/flowcat_output_manifest.json" \
  --outdir "$CAROTID_OUT/qc"
```

When analysis volumes exist, add `--labelled-artery`, `--shell`, `--tissue`,
and `--calcium` overlays. QC output always remains visual-review-pending until a
reviewer records a decision.

See `docs/CURRENT_PIPELINE_AUDIT.md`, `docs/FLOWCAT_INTEGRATION.md`,
`docs/PERIVASCULAR_ARCHITECTURE.md`, `docs/DATA_DICTIONARY.md`,
`docs/VALIDATION_PLAN.md`, and `docs/REPRESENTATIVE_QC_REPORT.md` for the audited
state, methods, schemas, validation plan, and current representative result.
